...
Fixed version with proper timeout handling and error management
"""
...
class ReplicateAIEnhancer:
    ...
    def __init__(self, api_token: Optional[str] = None):
        ...
        if not self.api_token:
            raise ValueError("REPLICATE_API_TOKEN not found. Please set it as an environment variable.")
        
        # Set the token in environment if not already there
        ...
        logger.info("🚀 Replicate AI Enhancer initialized with token")
    
    def _run_with_timeout(self, model_key: str, input_data: Dict[str, Any]) -> Any:
        """
        Run a Replicate model with timeout and retry logic
        
        Args:
            model_key: Key for the model in self.models
            input_data: Input parameters for the model
            
        Returns:
            Model output or raises exception
        """
        model_id = self.models[model_key]
        timeout = self.timeouts.get(model_key, 30)
        
        try:
            # Create prediction
            logger.info(f"Starting {model_key} prediction...")
            ...
            while prediction.status not in ["succeeded", "failed", "canceled"]:
                if time.time() - start_time > timeout:
                    logger.error(f"Timeout after {timeout}s for {model_key}")
                    # Try to cancel the prediction
                    try:
                        self.client.predictions.cancel(prediction.id)
                    except:
                        pass
                    raise TimeoutError(f"Model {model_key} timed out after {timeout} seconds")
            ...
            logger.error(f"Error running {model_key}: {str(e)}")
            raise
    ...
    def analyze_market_intelligence(self, query: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Run Llama 2 70B with timeout
            output = self._run_with_timeout('llama2_70b', {
                "prompt": prompt,
            ...
            logger.error(f"Llama 2 timeout: {str(e)}")
            return {
                'status': 'error',
    ...
    def analyze_property_image(self, image_url: str, questions: List[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for question in questions:
                try:
                    output = self._run_with_timeout('blip2', {
                        "image": image_url,
                        "question": question
                    })
                    ...
                    results[question] = "Analysis timed out"
                except Exception as e:
    ...
    def generate_development_visualization(self, prompt: str, property_data: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            output = self._run_with_timeout('stable_diffusion', {
            ...
            logger.error("Stable Diffusion timeout")
            return {
                'status': 'error',
                'error': 'Image generation timed out',
                'fallback': 'Unable to generate visualization at this time'
            }
        except Exception as e:
    ...
    def analyze_investment_opportunity(self, property_data: Dict[str, Any], market_data: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            output = self._run_with_timeout('mistral', {
                "prompt": prompt,
                "temperature": 0.2,
                "max_tokens": 400,
                "top_p": 0.9
            ...
            response = "".join(str(item) for item in output) if isinstance(output, list) else str(output)
...
def test_replicate_enhancer():
    ...
    print("🧪 Testing Replicate AI Enhancer with Timeout Handling...")
    print("="*60)
    
    try:
        enhancer = ReplicateAIEnhancer()
        print("✅ Token validated and client initialized")
    ...
    if market_result['status'] == 'success':
        ...
        print(f"Error: {market_result.get('error', 'Unknown error')}")
    ...
    if investment_result['status'] == 'success':
        ...
        print(f"Error: {investment_result.get('error', 'Unknown error')}")
    ...
    print("\n✅ Replicate AI Enhancer with timeout handling ready for production!")
